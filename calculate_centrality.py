...
plt.figure(figsize=(10,10),facecolor=[0.7,0.7,0.7,0.4])
nx.draw_networkx(G, pos=pos, with_labels=True, node_color=c, node_size=s,edge_color= ec,width=ew,font_color='white',font_weight='bold',font_size='6')
plt.axis('off')
plt.show()
"""
...
graph_colormap = cm.get_cmap('plasma', 9)
...
s =  rescale([v for v in bc.values()],500,1000)
# edge width shows 1-weight to convert cost back to strength of interaction 
ew = rescale([float(G[u][v]['weight']) for u,v in G.edges],0.1,0.5)
# edge color also shows weight
ec = rescale([float(G[u][v]['weight']) for u,v in G.edges],0.1,0.5)
ec = [graph_colormap(i) for i in ec]
plt.figure(figsize=(12, 8))

pos = nx.spring_layout(G)
nx.draw_networkx(G, pos=pos, with_labels=True, node_color=c, node_size=s, edge_color=ec, width=ew, font_color='white', font_weight='bold', font_size='6', alpha=0.7, label='Protein-Protein Interaction')

# Plot the drug-protein interaction graph on top of the protein-protein interaction graph
pos1 = nx.spring_layout(dp_G)
nx.draw_networkx(dp_G, pos=pos1, with_labels=True, node_color='green', node_size=100, font_color='black', font_weight='bold', font_size='6', alpha=0.7, label='Drug-Protein Interaction')
...
top_nodes = sorted(G_node_degrees.items(), key=lambda x: x[1], reverse=True)[:5]

# Extract node names from the top nodes
top_node_names = [node[0] for node in top_nodes]

print("Top 5 nodes with the most interactions:")
i=1
for node in top_nodes:
    print(f"Node {i}: {node[0]}, Degree: {node[1]}")
    i+=1
    
dp_G_node_degrees = dict(dp_G.degree())

# Find the top 5 nodes with the highest degrees
top_nodes = sorted(dp_G_node_degrees.items(), key=lambda x: x[1], reverse=True)[:5]
