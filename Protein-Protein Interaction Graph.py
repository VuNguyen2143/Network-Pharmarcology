...
nx.draw_networkx(G, pos=pos, with_labels=True, node_color=c, node_size=s,edge_color= ec,width=ew,font_color='white',font_weight='bold',font_size='4')
plt.axis('off')
plt.show()
...
# Find the top 10 nodes with the highest degrees
...
for node in top_nodes:
    topological_coefficient = nx.clustering(G, node)
    print(f"Node {i}: {node[0]}, Interaction: {node[1]}, Degree Centrality: {degree_centrality[node[0]]}, Betweenness Centrality: {betweenness_centrality[node[0]]}, Closeness Centrality: {closeness_centrality[node[0]]}, Clustering Coefficient: {clustering_coefficient[node[0]]}, Topological Coefficient: {topological_coefficient} ")
    i+=1
